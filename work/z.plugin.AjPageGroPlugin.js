(function(window, z, plugin, $, undefined) {
    if (z && plugin && !plugin.AjPageGroPlugin) {
                ...
                if (datagrid != null && datagrid.rows != null && datagrid.rows.length > 0) {
                    ...
                    for (var i = 0, l = datagrid.rows.length; i < l; i++) {
                        if (datagrid.rows[i] != null) {
                            ...
                            // 研判区 讨论区 会商区
                            datagrid.rows[i].ypq = "<li class=\"li1\" onclick=\"z.plugin.YpxxPlugin.instance.init_page('" +
                                datagrid.rows[i].id + "','ypq');\"><span>研判区</span><b class=\"yanpan\">" +
                                datagrid.rows[i].ajyps +
                                "</b></li>";
                            datagrid.rows[i].tlq = "<li class=\"li2\" onclick=\"z.plugin.YpxxPlugin.instance.init_page('" +
                                datagrid.rows[i].id +
                                "','tlq');\"><span>讨论区</span><b class=\"taolun\">" +
                                datagrid.rows[i].ajtls +
                                "</b></li>";
                            if (datagrid.rows[i].jqhs == false) {
                                datagrid.rows[i].hsq = "<li class=\"li3\" onclick=\"z.plugin.YpxxPlugin.instance.init_page('" +
                                    datagrid.rows[i].id +
                                    "','hsq');\"><span>会商区</span><b class=\"huishang\">" +
                            ...
                            if (datagrid.rows[i].ajypcs != null && datagrid.rows[i].ajypcs != "") {
                                if (Number(datagrid.rows[i].ajypcs) > 1) {
                                    datagrid.rows[i].ypcs = "[" + z.plugin.SetUtilsPlugin.instance.convertToChinese(datagrid.rows[i].ajypcs) + "次研判]";
                                } else {
                            ...
                            if (datagrid.rows[i].ypBgZt != null) {
                                if (datagrid.rows[i].ypBgZt) {
                                    datagrid.rows[i].ypBgZt = "<div  onclick=\"z.plugin.AjypPlugin.instance.laodYpBg('" +
                                        datagrid.rows[i].ajypId +
                                        "')\" class='yp_hyjy'><div class='huiyi'>研判</div><div class='jiyao'>报告</div></div>";
                                } else {
                                    datagrid.rows[i].ypBgZt = "<div class='yp_hyjy2'><div class='huiyi'>研判</div><div class='jiyao'>报告</div>" +
                                        "<i class='iconfont' onclick=\"z.plugin.AjypPlugin.instance.laodYpBg_Editpage('" +
                                        datagrid.rows[i].ajypId +
                                        "')\"></i></div>";
                                }
                            } else {
                    ...
                    success: function(datagrid) {
                        ...
                        for (var i = 0, l = datagrid.rows.length; i < l; i++) {
                            if (datagrid.rows[i] != null) {
                                ...
                                datagrid.rows[i].ypq = "<li class=\"li1\" onclick=\"z.plugin.YpxxPlugin.instance.init_page('" +
                                    datagrid.rows[i].id +
                                    "','ypq');\"><span>研判区</span><b class=\"yanpan\">" +
                                    datagrid.rows[i].ajyps +
                                    "</b></li>";
                                datagrid.rows[i].tlq = "<li class=\"li2\" onclick=\"z.plugin.YpxxPlugin.instance.init_page('" +
                                    datagrid.rows[i].id +
        ...
        z.getInstance("z.plugin.AjPageGroPlugin")
    }
})(window, z, z.plugin, $);